...
<!-- Example DataTables Card-->
      <div class="card mb-3">
        <div class="card-header">
          <i class="fa fa-file-text-o"></i> Data Penjualan Telur</div>
        <div class="card-body">
...
<a data-toggle="tooltip" data-placement="bottom" title="Menambah Pelanggan" href="Pelanggan.php"><button class="AddSkh"><i class="fa fa-plus"></i> Tambah Pelanggan</button></a>
<a data-toggle="tooltip" data-placement="bottom" title="Menambah Penjualan" href="#"><button class="AddSkh" data-toggle="modal" data-target="#modalAddMDL"><i class="fa fa-plus"></i> Tambah Penjualan</button></a>
            <table class="table table-bordered table-a" id="dataTable" width="100%" cellspacing="0">
              ...
              <tbody>
          <?php
            $sql="select penjualan_telur.*, customer.nama FROM customer inner join penjualan_telur on customer.id_customer=penjualan_telur.id_customer";
            $query = mysql_query($sql);
            while ($View=mysql_fetch_array($query)) { 

              $TGL = $View['tanggal_jual'];
              $ForTGLJual=date('d-M-Y', strtotime($TGL));
              $HRG=$View['harga_perbtr'];
              $HARGA=number_format($HRG);
              $TOT=$View['total_pjl'];
              $TOTAL_PJL=number_format($TOT);
              $PMB=$View['pembayaran'];
              $PEMBAYARAN=number_format($PMB);
          ?> 
                <tr>
                  <td><?php echo $View['id_penjualan']; ?></td>
                  <td><?php echo $ForTGLJual; ?></td>
                  <td><?php echo $View['nama']; ?></td>
                  <td><?php echo $View['jml_telur']; ?> Butir</td>
                  <td>Rp. <?php echo $HARGA; ?></td>
                  <td>Rp. <?php echo $TOTAL_PJL; ?></td>
                  <td>Rp. <?php echo $PEMBAYARAN; ?></td>
                  <td><?php echo $View['keterangan']; ?></td>
                  <td>
                    <a data-toggle="tooltip" data-placement="right" title="Edit" href="#" class='OpenModal' id='<?php echo $View['id_penjualan']; ?>'><button class='butEdit' id='Edit'><i class='fa fa-edit'></i></button></a>
                    <a data-toggle="tooltip" data-placement="right" title="Hapus" href="#" onclick="confirm_modal('DELETE/delete-Penjualan.php?&id_penjualan=<?php echo  $View['id_penjualan']; ?>');"><button class="butDelete"><i class="fa fa-trash"></i></button></a>
                  </td>
                </tr>
            <?php } ?>  
              </tbody>
...
<script type="text/javascript">
   ...
   $(".OpenModal").click(function(e) {
      var m = $(this).attr("id");
       $.ajax({
             url: "EDIT-DATA/Edit-Penjualan.php",
             type: "GET",
             data : {id_penjualan: m,},
...
<!-- Modal Add Penjualan Telur -->
<?php   
  $ID_PROFIT=1;
  $sql2="select * FROM penjualan_telur";
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-header" style="background-color: #DDDDDD;">
                <h3 class="modal-title"  id="myModalLabel" style="text-align: center;">Tambah Penjualan Telur <i class="fa fa-pencil"></i></h3>
            ...
            <div class="modal-body">
                <p class="statusMsg"></p>
                <form role="form" id="FormValidation" method="post" action="SIMPAN-DATA/Proses-Save-Penjualan.php">
                <input type="hidden" name="id_penjualan" value=<?php echo $query2; ?>>
                <input type="hidden" name="id_profit" value=<?php echo $ID_PROFIT; ?>> 
                    
                    <div class="form-group">
                      <label for="nama" class="labelfrm">Pelanggan</label>
                        <div class="input-group">
                          <span class="input-group-addon" id="basic-addon1"><span class="fa fa-user"></span></span>
                          <select class="form-control" name="id_customer" id="id_customer" required="true">
                      ...
                      <label for="tanggal_jual" class="labelfrm">Tanggal Penjualan</label>
                        <div class="input-group">
                          ...
                          <input id="tanggal_jual" class="form-control" type="date" name="tanggal_jual" required="true" autocomplete="off">
                        </div>
                      <label for="jml_telur" class="labelfrm">Jumlah Telur</label>
                        <div class="input-group">
                          ...
                          <input id="jml_telur" class="form-control" type="text" placeholder="Jumlah Telur" name="jml_telur" required="true" autocomplete="off" onkeypress="return mungAngka(event)">
                        </div>
                      <label for="harga_perbtr" class="labelfrm">Harga (/Butir)</label>
                        <div class="input-group">
                          ...
                          <input id="harga_perbtr" class="form-control" type="text" placeholder="Harga (/Butir)" name="harga_perbtr" autocomplete="off" required="true" onkeypress="return mungAngka(event)">
                        </div>
                      <label for="pembayaran" class="labelfrm">Pembayaran</label>
                        <div class="input-group">
                          ...
                          <input id="pembayaran" class="form-control" type="text" placeholder="Pembayaran" name="pembayaran" autocomplete="off" required="true" onkeypress="return mungAngka(event)">
                        </div>
