...
<!-- Breadcrumbs-->
      <ol class="breadcrumb">
        <li class="breadcrumb-item">
          <a data-toggle="tooltip" data-placement="bottom" title="Halaman Utama Modal Keluar" href="Home-Modal.php">Modal Keluar</a>
        </li>
...
<!-- Example DataTables Card-->
      <div class="card mb-3">
        <div class="card-header">
          <i class="fa fa-file-text-o"></i> Data Modal Bibit</div>
        <div class="card-body">
...
<a data-toggle="tooltip" data-placement="bottom" title="Menambah Modal" href="#"><button class="AddSkh" data-toggle="modal" data-target="#modalAddMDL"><i class="fa fa-plus"></i> Tambah Modal</button></a>
          ...
          <?php 
            $no=0;
            $sql="select * FROM m_bibit ORDER BY tgl_bibit DESC";
            $query = mysql_query($sql);
            while ($View=mysql_fetch_array($query)) {
            $no++; 

            $HRG=$View['harga'];
            $HargaBeli=number_format($HRG);
            $TOT=$View['total_Mbibit'];
            $TotalModal=number_format($TOT);
            $TGL = $View['tgl_bibit'];
            $ForTGLMasuk=date('d-M-Y', strtotime($TGL));          
          ?> 
                <tr>
                  <td><?php echo $no; ?></td>
                  <td><?php echo $View['nama']; ?></td>
                  <td><?php echo $View['populasi']; ?> ekor</td>
                  <td>Rp. <?php echo $HargaBeli; ?></td>
                  <td><?php echo $ForTGLMasuk; ?></td>
                  <td>Rp. <?php echo $TotalModal; ?></td>
                  <td>
                    <a data-toggle="tooltip" data-placement="bottom" title="Edit" href="#" class='OpenModal' id='<?php echo $View['id_mbibit']; ?>'><button class='butEdit' id='Edit'><i class='fa fa-edit'></i></button></a>
                    <a data-toggle="tooltip" data-placement="bottom" title="Hapus" href="#" onclick="confirm_modal('DELETE/delete-MBibit.php?&id_mbibit=<?php echo  $View['id_mbibit']; ?>');"><button class="butDelete"><i class="fa fa-trash"></i></button></a>
                  </td>
                </tr>
            <?php } ?>  
              </tbody>
...
<!-- TAMBAH MODAL BIBIT -->
<!-- Modal Add Modal Bibit -->
<?php   
  $sql2="select * FROM m_bibit";
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-header" style="background-color: #DDDDDD;">
                <h3 class="modal-title"  id="myModalLabel" style="text-align: center;">Tambah Modal <i class="fa fa-pencil"></i></h3>
            ...
            <div class="modal-body">
                <p class="statusMsg"></p>
                <form role="form" id="FormValidation" method="post" action="SIMPAN-DATA/Proses-Save-Mbibit.php">
                <input type="hidden" name="id_mbibit" value=<?php echo $query2; ?>>
                    ...
                    <div class="form-group">
                      ...
                      <label for="populasi" class="labelfrm">Jumlah Populasi (ekor)</label>
                        <div class="input-group">
                          ...
                          <input id="populasi" class="form-control" type="text" placeholder="Jumlah Populasi (ekor)" name="populasi" required="true" autocomplete="off" onkeypress="return mungAngka(event)">
                        </div>
                      <label for="harga" class="labelfrm">Harga (/Ekor)</label>
                        <div class="input-group">
                          ...
                          <input id="harga" class="form-control" type="text" placeholder="Harga (/Ekor)" name="harga" required="true" autocomplete="off" onkeypress="return mungAngka(event)">
                        </div>
                      <label for="tgl_bibit" class="labelfrm">Tanggal (Masuk)</label>
                        <div class="input-group">
                          ...
                          <input id="tgl_bibit" class="form-control" type="date" name="tgl_bibit" required="true" autocomplete="off">
                        </div>
                    </div>

                  <div class="modal-footer">
            <button data-toggle="tooltip" data-placement="bottom" title="Simpan" type="submit" class="btn btn-primary buttonLog">Simpan <i class="fa fa-check"></i></button></a>
            <button data-toggle="tooltip" data-placement="bottom" title="Batal" type="reset" class="btn btn-default buttonBatt" data-dismiss="modal">Batal <i class="fa fa-close"></i></button>
                  </div>
                </form>
            </div>
