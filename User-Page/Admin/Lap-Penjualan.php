...
<!-- Example DataTables Card-->
      <div class="card mb-3">
        <div class="card-header">
          <i class="fa fa-file-text-o"></i> Laporan Penjualan Telur</div>
        <div class="card-body">
...
-webkit-transform:scale(1.1);
  ...
  background-color: #005ca7;
                ...
                <form id="FormValidation" method="post" action="Lap-Penjualan.php">
...
?>
<a data-toggle="tooltip" data-placement="bottom" title="Klik untuk Mencetak" href="../../Laporan/Rep-Penjualan.php?awal=<?php echo $tanggal_awal; ?>&akhir=<?php echo $tanggal_akhir; ?>"><button class="AddSkh"><i class="fa fa-print"></i> Print Laporan Modal</button></a>
<?php 
    $sql1="select SUM(pembayaran) AS tot_penjualan  FROM penjualan_telur WHERE tanggal_jual between '$tanggal_awal' and '$tanggal_akhir'";
    $query1 = mysql_query($sql1);
    $View1=mysql_fetch_array($query1);

      $TOT=$View1['tot_penjualan'];
      $TOTAL_KABEH=number_format($TOT);

    $sql2="select SUM(jml_telur) AS tot_telur_terjual  FROM penjualan_telur where tanggal_jual BETWEEN '$tanggal_awal' and '$tanggal_akhir'";
...
?>
<h6>Total Penjualan dari tanggal <i><?php echo $ForTglAWAL; ?></i> sampai tanggal <i><?php echo $ForTglAKHIR; ?></i> : <b>Rp. <?php echo $TOTAL_KABEH; ?></b></h6>
<h6>Total Telur Terjual dari tanggal <i><?php echo $ForTglAWAL; ?></i> sampai tanggal <i><?php echo $ForTglAKHIR; ?></i> : <b><?php echo $TOT_TELUR_JUAL; ?> Butir</b></h6><br>
            ...
            <!-- Data Modal Pakan -->
              ...
              <tbody>
          <?php 
            $sql="select penjualan_telur.*, customer.nama FROM customer inner join penjualan_telur on customer.id_customer=penjualan_telur.id_customer WHERE (penjualan_telur.tanggal_jual between '$tanggal_awal' and '$tanggal_akhir')";
